import type { NextApiRequest, NextApiResponse } from 'next';
import { roqClient } from 'server/roq';
import { prisma } from 'server/db';
import { errorHandlerMiddleware } from 'server/middlewares';
import { galleryValidationSchema } from 'validationSchema/galleries';
import { HttpMethod, convertMethodToOperation, convertQueryToPrismaUtil } from 'server/utils';
import { getServerSession } from '@roq/nextjs';

async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { roqUserId, user } = await getServerSession(req);
  await prisma.gallery
    .withAuthorization({
      roqUserId,
      tenantId: user.tenantId,
      roles: user.roles,
    })
    .hasAccess(req.query.id as string, convertMethodToOperation(req.method as HttpMethod));

  switch (req.method) {
    case 'GET':
      return getGalleryById();
    case 'PUT':
      return updateGalleryById();
    case 'DELETE':
      return deleteGalleryById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getGalleryById() {
    const data = await prisma.gallery.findFirst(convertQueryToPrismaUtil(req.query, 'gallery'));
    return res.status(200).json(data);
  }

  async function updateGalleryById() {
    await galleryValidationSchema.validate(req.body);
    const data = await prisma.gallery.update({
      where: { id: req.query.id as string },
      data: {
        ...req.body,
      },
    });

    return res.status(200).json(data);
  }
  async function deleteGalleryById() {
    const data = await prisma.gallery.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
}

export default function apiHandler(req: NextApiRequest, res: NextApiResponse) {
  return errorHandlerMiddleware(handler)(req, res);
}
